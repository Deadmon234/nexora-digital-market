  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-4">
        {product.brandName && (
          <p className="text-xs font-medium uppercase tracking-wide text-slate-500">{product.brandName}</p>
        )}
        <h3 className="mt-1 font-semibold text-slate-900 group-hover:underline">{product.name}</h3>
        <p className="mt-2 line-clamp-2 flex-1 text-sm text-slate-600">{product.description}</p>
        <div className="mt-4 flex items-center justify-between">
          <span className="text-lg font-bold text-slate-900">{formatPrice(product.minPrice)}</span>
